...
module.exports = CoreObject.extend({
  ...
  upload: function(options) {
    ...
    console.log('?  CHECKING REVISIONS ' + revisionKey + ' ' + JSON.stringify(options, null, 2));

    return this.fetchRevisions(options)
      .then(function(revisions) {
        console.log('?  LOADED REVISIONS ' + JSON.stringify(revisions, null, 2));
        var found = revisions.map(function(element) { return element.revision; }).indexOf(options.revisionKey);
        console.log('?  FOUND ' + found + ' ' + JSON.stringify(options, null, 2));

        if (found >= 0 && !allowOverwrite) {
          // plugin.log('?  REVISION ALREADY UPLOADED' , { verbose: true });
          console.log('REVISION ALREADY UPLOADED');
