...
module.exports = CoreObject.extend({
  ...
  upload: function(options) {
    ...
    plugin.log('?  CHECKING REVISIONS ' + revisionKey + ' ' + JSON.stringify(options, null, 2), { verbose: true });

    return this.fetchRevisions(options)
      .then(function(revisions) {
        plugin.log('?  LOADED REVISIONS ' + JSON.stringify(revisions, null, 2), { verbose: true });
        var found = revisions.map(function(element) { return element.revision; }).indexOf(options.revisionKey);
        plugin.log('?  FOUND ' + found + ' ' + JSON.stringify(options, null, 2), { verbose: true });
